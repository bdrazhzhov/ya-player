...
class Block {
  ...
  Block(
      ...
      this.description,
      required this.type,
      required this.typeForFrom,
      this.viewAllUrl,
      this.viewAllUrlScheme,
  ...
  factory Block.fromJson(Map<String, dynamic> json) {
    ...
    switch (type) {
      case 'personal-playlists':
        json['entities'].forEach((entity) =>
            entities.add(Playlist.fromJson(entity['data']['data'])));
      case 'play-contexts':
        json['entities']
            .forEach((entity) => _createPlayContext(entity, entities));
      case 'promotions':
      ...
      case 'podcasts':
        json['entities']
            .forEach((entity) => entities.add(Podcast.fromJson(entity)));
      case 'new-releases':
        json['entities']
            .forEach((entity) => entities.add(Album.fromJson(entity['data'])));
      case 'new-playlists':
        json['entities'].forEach(
            (entity) => entities.add(Playlist.fromJson(entity['data'])));
      case 'chart':
        json['entities'].forEach((entity) =>
            entities.add(Track.fromJson(entity['data']['track'], '')));
      case 'mixes':
        json['entities'].forEach(
            (entity) => entities.add(MixLink.fromJson(entity['data'])));
      case 'editorial-playlists':
        json['entities']
            .forEach((entity) => _createPodcastEntity(entity, entities));
      case 'album-chart':
        json['entities'].forEach((entity) =>
            entities.add(Podcast.fromJson(entity['data']['album'])));
      case 'playlist-with-tracks':
        json['entities'].forEach(
            (entity) => entities.add(Playlist.fromJson(entity['data'])));
      case 'recently-played':
        json['entities']
            .forEach((entity) => _createRecentlyPlayed(entity, entities));
      default:
    ...
    return Block(
        ...
        viewAllUrlScheme: json['viewAllUrlScheme'],
        entities: entities);
  }
  ...
  static void _createRecentlyPlayed(entity, List<Object> entities) {
    switch (entity['type']) {
      case 'album':
        if (entity['data']['type'] == 'podcast' ||
            entity['data']['type'] == 'audiobook') {
  ...
  static void _createPodcastEntity(
      Map<String, dynamic> entityJson, List<Object> entities) {
